...
describe('TechnicianService', () => {
 ...
 describe('updateTechnician', () => {
  it('should update and return the technician', async () => {
    ...
    repo.save = jest.fn().mockResolvedValue({ ...mockTechnician, name: 'Updated' });

    const result = await service.updateTechnician('SN123', { ...mockCreateDto, name: 'Updated' });
    ...
    it('should throw NotFoundException if not found', async () => {
      ...
      await expect(service.updateTechnician('SN404', { ...mockCreateDto, name: 'X' })).rejects.toThrow(NotFoundException);
    });

    it('should throw InternalServerErrorException on error', async () => {
      jest.spyOn(service, 'findOneTechnician').mockResolvedValue(mockTechnician);
      repo.save = jest.fn().mockRejectedValue(new Error());
      await expect(service.updateTechnician('SN123', { ...mockCreateDto, name: 'X' })).rejects.toThrow(InternalServerErrorException);
    });
  });
